...
class EDFExporter:
  ...
  def export_unified_edf(
    ...
    """Exporta el archivo EDF+ completo exigiendo metadatos."""
    ...
    for i, h in enumerate(headers):
      ...
      raw_fs = h.get('sample_frequency') or h.get('sample_rate')
      if raw_fs is None or float(raw_fs) <= 0:
        raise ValueError(
            f"El canal {i+1} ('{h.get('label', f'Ch_{i+1}')}') no especifica"
            ' una frecuencia de muestreo válida para exportar.'
        )
      fs_val = int(float(raw_fs))

      unit_val = h.get('dimension') or h.get('units')
      if not unit_val or not str(unit_val).strip():
        raise ValueError(
            f"El canal {i+1} ('{h.get('label', f'Ch_{i+1}')}') no especifica"
            ' dimensión o unidad para exportar.'
        )
      unit_str = str(unit_val).strip()[:8]
    ...
    writer = pyedflib.EdfWriter(
        output_path, n_channels, file_type=pyedflib.FILETYPE_EDFPLUS
    )
    ...
    formatted_signals = [
        np.ascontiguousarray(sig, dtype=np.float64) for sig in signals
    ]
